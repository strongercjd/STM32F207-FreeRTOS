...
/*Task1*/
#define TASK1_PRIO		2
#define TASK1_SIZE 		250  
TaskHandle_t Task1_Handler;
void TASK1_entry(void *pvParameters);

/*Task2*/
#define TASK2_PRIO		3
#define TASK2_SIZE 		250  
...
void TASK1_entry(void *pvParameters)
{
  ...
  while(1)
  {
    if(num < 100)
    {
      ...
      {
        printf("TASK1 xTaskNotify is %d\r\n",NotifyValue);
      }else
      {
        printf("TASK1 xTaskNotify is ERR\r\n");
      }
      taskEXIT_CRITICAL();
...
void TASK2_entry(void *pvParameters)
{
  ...
  while(1)
  {
    if(num < 100)
    {
      num ++;
      NotifyValue = num;
      err=xTaskNotify((TaskHandle_t	)Task1_Handler,		 //接收任务通知的任务句柄
                      (uint32_t		)NotifyValue,		 //任务通知值
                      (eNotifyAction	)eSetValueWithOverwrite);//覆写的方式发送任务通知
      taskENTER_CRITICAL();
      if(err==pdFAIL)
      {
        printf("TASK2 xTaskNotify ERR\r\n");
      }
      else
      {
        printf("TASK2 xTaskNotify OK\r\n");
      }
      taskEXIT_CRITICAL();
      vTaskDelay(500);
    }
