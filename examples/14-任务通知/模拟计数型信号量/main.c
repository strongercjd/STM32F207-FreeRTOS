...
void TASK1_entry(void *pvParameters)
{
  ...
  while(1)
  {
    ...
    {
      num ++;
      NotifyValue = ulTaskNotifyTake(pdFALSE,portMAX_DELAY);	//获取数值信号量值
      taskENTER_CRITICAL();
      printf("TASK2 COUNT_SEM Value:%d\r\n",NotifyValue);
      taskEXIT_CRITICAL();
      vTaskDelay(500);
    }
    else
...
void TASK2_entry(void *pvParameters)
{
  ...
  while(1)
  {
    ...
    {
      num ++;
      xTaskNotifyGive(Task1_Handler);//释放计数型信号量
      taskENTER_CRITICAL();
      printf("TASK2 xTaskNotifyGive\r\n");
      taskEXIT_CRITICAL();
      if(num < 50)
      {
        vTaskDelay(300);
